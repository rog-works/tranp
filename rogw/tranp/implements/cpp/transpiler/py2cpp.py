...
class Py2Cpp(ITranspiler):
	...
	def to_accessible_name_for_alt_class(self, raw: IReflection, var_type: str) -> str:
		...
		if not self.cvars.equals(self.cvars.to_type(raw.attrs[0]), CVars.Types.CRaw):
	...
	def allow_move_assign(self, value_raw: IReflection, declared: bool) -> bool:
		...
		cvar_type = self.cvars.to_type(var_type_raw.impl(refs.Object).actualize())
	...
	def proc_for_dict(self, node: defs.For, symbols: list[str], for_in: str, statements: list[str]) -> str:
		...
		is_const = self.cvars.contains(self.cvars.to_type(for_in_symbol), CVars.Types.Const) if len(symbols) == 1 else False
		# 期待値: 'iterates.items()'
	...
	def proc_for_each(self, node: defs.For, symbols: list[str], for_in: str, statements: list[str]) -> str:
		...
		for_in_symbol = Defer.new(lambda: self.reflections.type_of(node.for_in).impl(refs.Object).actualize())
		is_const = self.cvars.contains(self.cvars.to_type(for_in_symbol), CVars.Types.Const) if len(symbols) == 1 else False
		is_addr_raw = self.cvars.contains(self.cvars.to_type(for_in_symbol), CVars.Types.AddrRawMask) if len(symbols) == 1 else False
	...
	def on_relay(self, node: defs.Relay, receiver: str) -> str:
		...
		if self.is_relay_literalizer(node, receiver_symbol):
			...
			# 期待値: receiver.on
			cvar_type = self.cvars.to_type(receiver_symbol)
			...
			cvar_type = self.cvars.to_type(org_receiver_symbol.context)
			operator = self.cvars.to_operator(cvar_type).name
			prop = self.to_domain_name_by_class(prop_symbol.types) if isinstance(prop_symbol.decl, defs.Method) else self.to_prop_name(prop_symbol)
			is_property = isinstance(prop_symbol.decl, defs.Method) and prop_symbol.decl.is_property
	...
	def is_relay_cvar(self, node: defs.Relay, receiver_symbol: IReflection) -> bool:
		...
		cvar_type = self.cvars.to_type(receiver_symbol)
		return not self.cvars.equals(cvar_type, CVars.Types.CRaw)
	...
	def analyze_indexer_spec(self, node: defs.Indexer) -> 'tuple[IndexerSpec.Tags, IReflection | None]':
		...
		elif self.cvars.contains(self.cvars.to_type(receiver_symbol), CVars.Types.AddrRawMask):
	...
	def on_func_call(self, node: defs.FuncCall, calls: str, arguments: list[str]) -> str:
		...
		if spec == FuncCallSpec.Tags.c_include:
			...
			operator = '->' if node.arguments[0].value.is_a(defs.ThisRef) or self.cvars.contains(self.cvars.to_type(receiver_raw), CVars.Types.AddrMask) else '.'
			...
			return self.view.render(f'{node.classification}/{spec.name}', vars={**func_call_vars, 'receiver': receiver, 'is_addr': self.cvars.contains(cvar_type, CVars.Types.AddrMask)})
	...
	def analyze_func_call_spec(self, node: defs.FuncCall) -> 'tuple[FuncCallSpec.Tags, str, IReflection | None]':
		...
		if isinstance(node.calls, defs.Var):
			...
			elif not self.cvars.equals(self.cvars.to_type(calls_raw), CVars.Types.CRaw):
				# XXX AltClassを考慮するとRelay側も対応が必要で片手落ち
				cvar_type = self.cvars.to_type(calls_raw)
				return FuncCallSpec.Tags.cvar_to, self.cvars.type_to_name(cvar_type), None
			...
			if prop in FuncCallSpec.list_and_dict_methods:
				...
				cvar_type = self.cvars.to_type(receiver_raw)
				...
				receiver_raw = self.reflections.type_of(node.calls.receiver).impl(refs.Object).actualize()
				cvar_type = self.cvars.to_type(receiver_raw)
				if self.cvars.contains(cvar_type, CVars.Types.CRef):
				...
				if self.cvars.contains(cvar_type, CVars.Types.AddrSmartMask):
					...
					return FuncCallSpec.Tags.cvar_smart_empty, self.cvars.type_to_name(cvar_type), entity_raw
			elif prop == CVars.Verbs.New.value and isinstance(node.calls.receiver, defs.Var):
				receiver_raw = self.reflections.type_of(node.calls.receiver).impl(refs.Object).actualize()
				cvar_type = self.cvars.to_type(receiver_raw)
				if self.cvars.contains(cvar_type, CVars.Types.AddrRawMask):
					return FuncCallSpec.Tags.cvar_new_addr, '', None
				elif self.cvars.contains(cvar_type, CVars.Types.AddrSmartMask):
					new_type_raw = self.reflections.type_of(node.arguments[0]).impl(refs.Object)
					if new_type_raw.type_is(list):
						return FuncCallSpec.Tags.cvar_new_smart_list, self.cvars.type_to_name(cvar_type), new_type_raw

					return FuncCallSpec.Tags.cvar_new_smart, self.cvars.type_to_name(cvar_type), None
				...
				receiver_raw = self.reflections.type_of(node.calls.receiver).impl(refs.Object).actualize()
				cvar_type = self.cvars.to_type(receiver_raw)
				if not self.cvars.equals(cvar_type, CVars.Types.CRaw):
					return FuncCallSpec.Tags.cvar_to_addr_hex, self.cvars.type_to_name(cvar_type), None
			elif prop == CVars.Verbs.ToAddrId.value:
				receiver_raw = self.reflections.type_of(node.calls.receiver).impl(refs.Object).actualize()
				cvar_type = self.cvars.to_type(receiver_raw)
				if not self.cvars.equals(cvar_type, CVars.Types.CRaw):
					return FuncCallSpec.Tags.cvar_to_addr_id, self.cvars.type_to_name(cvar_type), None
