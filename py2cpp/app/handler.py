...
class Context:
	def __init__(self, register: Register, writer: Writer, view: Renderer) -> None:
...
class Handler:
	...
	def on_constructor(self, node: defs.Constructor, ctx: Context) -> None:
		_, block = ctx.register.pop(defs.Block, str)
		text = ctx.view.render('constructur.j2', indent=node.nest, vars={**serialize(node, T_FunctionVar), 'block': block})
		ctx.register.push(node, text)

	def on_class_method(self, node: defs.ClassMethod, ctx: Context) -> None:
		_, block = ctx.register.pop(defs.Block, str)
		text = ctx.view.render('class_method.j2', indent=node.nest, vars={**serialize(node, T_FunctionVar), 'block': block})
		ctx.register.push(node, text)

	def on_method(self, node: defs.Method, ctx: Context) -> None:
		_, block = ctx.register.pop(defs.Block, str)
		text = ctx.view.render('method.j2', indent=node.nest, vars={**serialize(node, T_FunctionVar), 'block': block})
		ctx.register.push(node, text)
