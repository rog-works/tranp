...
from rogw.tranp.implements.syntax.tranp.state import Context, StateStore, State, States, Trigger, Triggers
...
def build_state_stores(*states: State) -> list[StateStore]:
	state_stores: list[StateStore] = []
	for index, state in enumerate(states):
		if state == States.Idle:
			state_stores.append(StateStore())
		else:
			state_stores.append(StateStore(state, index, index))
...
class TestExpressionsOr(TestCase):
	@data_provider([
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.Idle}, Triggers.Empty),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.Step}, Triggers.Empty),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.FinishSkip}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.FinishStep}, Triggers.UnfinishStep),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.UnfinishSkip}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.UnfinishStep}, Triggers.UnfinishStep),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.Abort}, Triggers.Empty),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 0, 1, {'b': States.Idle}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 0, 1, {'b': States.Step}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 0, 1, {'b': States.FinishSkip}, Triggers.FinishSkip),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 0, 1, {'b': States.FinishStep}, Triggers.FinishStep),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 0, 1, {'b': States.UnfinishSkip}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 0, 1, {'b': States.UnfinishStep}, Triggers.UnfinishStep),
		(['a', 'b'], build_state_stores(States.FinishSkip, States.Idle), 0, 1, {'b': States.Abort}, Triggers.FinishSkip),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 0, 1, {'b': States.Abort}, Triggers.FinishSkip),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 0, 1, {'b': States.Idle}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 0, 1, {'b': States.Step}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 0, 1, {'b': States.FinishSkip}, Triggers.FinishSkip),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 0, 1, {'b': States.FinishStep}, Triggers.FinishStep),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 0, 1, {'b': States.UnfinishSkip}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 0, 1, {'b': States.UnfinishStep}, Triggers.UnfinishStep),
		(['a', 'b'], build_state_stores(States.UnfinishSkip, States.Idle), 0, 1, {'b': States.Abort}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 0, 1, {'b': States.Abort}, Triggers.UnfinishSkip),
		# XXX 同じ文字数で片側だけ先に確定する状況は本来あり得ないが、正しい状況を再現するのが手間なことと、検証したい状況と実質的にほぼ同等であるため一旦これで良しとする
	])
	def test_accept(self, expressions: list[str], state_stores: list[StateStore], cursor: int, token_no: int, on_states: dict[str, State], expected: Trigger) -> None:
		def state_of(name: str, state: State) -> bool:
			return name in on_states and state == on_states[name]
...
class TestExpressionsAnd(TestCase):
	@data_provider([
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.Idle}, Triggers.Empty),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.Step}, Triggers.Empty),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.FinishSkip}, Triggers.Skip),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.FinishStep}, Triggers.Step),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.UnfinishSkip}, Triggers.Skip),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.UnfinishStep}, Triggers.Step),
		(['a', 'b'], build_state_stores(States.Idle, States.Idle), 0, 0, {'a': States.Abort}, Triggers.Abort),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 1, 1, {'b': States.Idle}, Triggers.Empty),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 1, 1, {'b': States.Step}, Triggers.Empty),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 1, 1, {'b': States.FinishSkip}, Triggers.FinishSkip),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 1, 1, {'b': States.FinishStep}, Triggers.FinishStep),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 1, 1, {'b': States.UnfinishSkip}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 1, 1, {'b': States.UnfinishStep}, Triggers.UnfinishStep),
		(['a', 'b'], build_state_stores(States.FinishStep, States.Idle), 1, 1, {'b': States.Abort}, Triggers.Abort),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 1, 1, {'b': States.Idle}, Triggers.Empty),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 1, 1, {'b': States.Step}, Triggers.Empty),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 1, 1, {'b': States.FinishSkip}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 1, 1, {'b': States.FinishStep}, Triggers.UnfinishStep),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 1, 1, {'b': States.UnfinishSkip}, Triggers.UnfinishSkip),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 1, 1, {'b': States.UnfinishStep}, Triggers.UnfinishStep),
		(['a', 'b'], build_state_stores(States.UnfinishStep, States.Idle), 1, 1, {'b': States.Abort}, Triggers.Abort),
	])
	def test_accept(self, expressions: list[str], state_stores: list[StateStore], cursor: int, token_no: int, on_states: dict[str, State], expected: Trigger) -> None:
		def state_of(name: str, state: State) -> bool:
			return name in on_states and state == on_states[name]
