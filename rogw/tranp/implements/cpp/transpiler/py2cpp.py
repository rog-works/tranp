...
class Py2Cpp(ITranspiler):
	...
	def __build_include_dirs(self, options: TranspilerOptions) -> dict[str, str]:
		"""Args: options: 実行オプション Returns: インクルードディレクトリー一覧"""
	...
	def allow_move_assign(self, value_raw: IReflection, declared: bool) -> bool:
		...
		cvar_type = self.cvars.resolve_type(var_type_raw.impl(refs.Object).actualize())
	...
	def on_parameter(self, node: defs.Parameter, symbol: str, var_type: str, default_value: str) -> str:
		...
		return self.render(node, f'element/{node.classification}', vars={'symbol': symbol, 'var_type': var_type, 'default_value': default_value, 'packing': node.packing, 'annotations': annotations})
	...
	def on_var(self, node: defs.Var) -> str:
		...
		if org_symbol.type_is(type):
			return self.render(node, f'reference/{node.classification}', vars={'symbol': node.tokens, 'class_symbol': self.to_domain_name_by_class(actual_symbol.types), 'module_path': actual_symbol.types.module_path})
		# 上記以外のクラス系参照の場合
	...
	def on_class_ref(self, node: defs.ClassRef) -> str:
		...
		return self.render(node, f'reference/{node.classification}', vars={'class_symbol': self.to_domain_name(symbol)})
	...
	def analyze_indexer_spec(self, node: defs.Indexer) -> 'tuple[IndexerSpec.Tags, IReflection | None]':
		...
		elif self.cvars.contains(self.cvars.resolve_type(receiver_symbol), CVars.Types.AddrRawMask):
	...
	def analyze_func_call_spec(self, node: defs.FuncCall) -> 'tuple[FuncCallSpec.Tags, str, IReflection | None]':
		...
		if isinstance(node.calls, defs.Var):
			...
			if calls == isinstance.__name__ or calls == issubclass.__name__:
				...
				cvar_type, cvar_key = self.cvars.resolve(calls_raw)
