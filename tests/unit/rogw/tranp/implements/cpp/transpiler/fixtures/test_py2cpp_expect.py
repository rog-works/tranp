...
class DeclOps {
	...
	def list_slice(cls, symbol: str, begin: str, end: str, step: str, var_type: str) -> str:
		return '\n'.join([
			...
			f'	auto __begin = {symbol}.begin() + {begin if begin else "0"};',
			f'	auto __end = __begin + {end if end else f"{symbol}.size()"};',
			f'	for (auto __iter = __begin; __iter < __end; __iter += {step if step else "1"}) ' '{',
