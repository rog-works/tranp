...
from tranp.lang.sequence import last_index_of
from tranp.node.definition.accessor import to_access
from tranp.node.definition.element import Decorator, Parameter
from tranp.node.definition.literal import Comment, String
from tranp.node.definition.primary import CustomType, DeclClassVar, DeclLocalVar, Declable, InheritArgument, DeclThisParam, DeclThisVar, Type, TypesName
from tranp.node.definition.statement_simple import AnnoAssign, MoveAssign
from tranp.node.definition.terminal import Empty
from tranp.node.embed import Meta, accept_tags, actualized, expandable
from tranp.node.interface import IDomain, IScope
from tranp.node.node import Node, T_Node
from tranp.node.promise import IDeclare, StatementBlock
	...
	for node in block.statements:
		...
		if isinstance(node, (If, Try)):
			for in_block in node.having_blocks:
				decl_vars = {**decl_vars, **{key: value for key, value in collect_decl_vars(in_block, allow).items() if key not in decl_vars}}
		elif isinstance(node, (While, For)):
			decl_vars = {**decl_vars, **{key: value for key, value in collect_decl_vars(node.block, allow).items() if key not in decl_vars}}
