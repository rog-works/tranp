...
class ForClass:
	...
	class Alias:
		...
		class Inner:
			V: ClassVar[int] = 0
			anno_n: Annotated[int, Embed.alias('n')]
			anno_s: Annotated[str, 'meta', Embed.alias('s')]

			def __init__(self) -> None:
				self.anno_n = 0
				self.anno_s = '1'
				ns = self.anno_n + int(self.anno_s)
...
class ForFunction:
	...
	class Method:
		...
		def numbers(self) -> Iterator[int]:
			for index in range(len(self.lts)):
				yield self.lts[index][1]
	...
	class Modifier:
		...
		@Embed.pure
		def pure(self) -> None: ...
		def implicit_immutable(self, s_i: str, ns_i: list[int], dsn_i: dict[str, int], func_i: Callable[[], None]) -> None: ...
		def mod_immutable(self, n: Annotated[int, Embed.immutable], v: Annotated['ForFunction.Modifier', Embed.immutable]) -> None: ...
		def mod_reference(self, n: Annotated[int, Embed.reference], v: Annotated['ForFunction.Modifier', Embed.reference]) -> None: ...
		def mod_mutable(self, s_m: Annotated[str, Embed.mutable]) -> None: ...
...
class ForTemplate:
	class TClass(Generic[T]):
		...
		def method_t2(self, v2: T2) -> T2: ...
		def method_t_t2(self, v: T, v2: T2) -> T2: ...
		def method_t_t1[T1](self, v: T, v1: T1) -> tuple[T, T1]: ...

	class T2Class(Generic[T2]): ...
	class T3Class[T1, T2](TClass[T1]): ...
...
class ForLambda:
	class A(Generic[T]):
		def __init__(self, f: Callable[[T], None] | None) -> None: ...
		def call(self, f: Callable[[T], None] | None) -> None: ...
