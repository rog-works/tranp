...
class ForFlows:
	...
	def for_items(self, dsn: dict[str, int], psn: CP[dict[str, int]]) -> None:
		for key, value in {'a': 1}.items(): ...
		for key, value in dsn.items(): ...
		for key, value in psn.on.items(): ...

	def for_each(self, strs: list[str], dsn: dict[str, int], cps: list[CPConst[int]], ts: list[tuple[int, int, int]]) -> None:
		for s in strs: ...
